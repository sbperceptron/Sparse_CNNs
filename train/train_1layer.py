...
class Train_Model1(object):
	def __init__(self, batchsize,full_pcs_bin_paths, full_pcs_labels_paths,full_pcs_calibs_paths,car_positives,neg,neg_root,\
	resolution,epochs,lr,SGDmomentum,L2weightdecay,N,weights1,weights2,b1,b2,RFCar,pad,curr_epoch,ch1,f1,
	x,y,z,fvthresh,wdthresh,batchsizehnm):
		super(Train_Model1, self).__init__()
	...
	def train(self):
		...
		for epoch in range(curr_epoch, epochs):
			...
			if (epoch+1) %10 == 0 and epoch != 0:
				...
				FPcount=hnm_obj.hard_negative_mining()
			...
			results=pool.map(nn_model,work) # nn function in the begining of file
			pool.close()
			pool.join()
			##################################################################################
			...
			b_grads_1= sum(results[:,2])/(results.shape[0])
			b_grads_2= sum(results[:,3])/(results.shape[0])
			total_error=sum(results[:,4])/(results.shape[0])
			AVG_fwd_time=sum(results[:,5])/(results.shape[0])
			AVG_bckwd_time=sum(results[:,6])/(results.shape[0])
			...
			################################################################################
			##############              weights2            ################################
			sgd_mom_w2=((lr * (grads_2.T+ (2*L2weightdecay*weights2)))+(SGDmomentum*previous_w2_grad))
			...
			error=np.round((sum(results[:,4])/(batchsize)),2)
			telapsed+=np.round((end_time-start_time),2)
			
			with open(loss_path, "a") as f: 
				f.write(str(error)+","+str(telapsed)+"\n") 

		

		if (epoch+1) %1 == 0 and epoch != 0:
			path='/home/saichand/3D_CNN/vote3deep2layer/train_singlelayer/weights/'
			name=path+'epoch_'+str(epoch+1)+'.weights'
			np.savez(name,w1=weights1,w2=weights2,b1=b1,b2=b2)
...
if __name__=='__main__':
	...
	car_positives="../data/crops/train/positive/Cyclist/*.bin"#Cyclists Car
	neg="../data/crops/train/negative/Cyclist/*.bin"
	neg_root="../data/crops/train/negative/Cyclist"
	test=glob.glob(neg)
	print len(test)
	dlksl
	#########################################################################################################
	...
	lr,SGDmomentum,L2weightdecay,N,weights1,weights2,b1,b2,RFCar,pad,curr_epoch,channels1,num_filters1,
	x,y,z,fvthresh,wdthresh,batchsizehnm)
